    ...
    def get_portfolio(self, account_id: str) -> Account:
        ...
        for pos_data in response.get("positions", []):
            try:
                ...
                # Use `or {}` instead of a bare default — if the API returns an
                # explicit null for a nested object, .get("key", {}) still yields
                # None (the key exists with value None), and None.get(…) raises
                # AttributeError.  `or {}` coerces both None and absent to {}.
                cost_basis_obj      = pos_data.get("costBasis")      or {}
                instrument_gain_obj = pos_data.get("instrumentGain") or {}
                last_price_obj      = pos_data.get("lastPrice")      or {}

                position = Position(
                    instrument=instrument,
                    quantity=float(pos_data.get("quantity") or 0),
                    current_value=float(pos_data.get("currentValue") or 0),
                    cost_basis=float(cost_basis_obj.get("totalCost") or 0),
                    unrealized_pnl=float(instrument_gain_obj.get("gainValue") or 0),
                    unrealized_pnl_percent=float(
                        instrument_gain_obj.get("gainPercentage") or 0
                    ),
                    last_price=float(last_price_obj.get("lastPrice") or 0),
                    percent_of_portfolio=float(
                        pos_data.get("percentOfPortfolio") or 0
                    ),
                )
        ...
        buying_power = 0.0
        bp_data = response.get("buyingPower") or {}
        if isinstance(bp_data, dict):
            try:
                buying_power = float(bp_data.get("buyingPower") or 0)
            except (ValueError, TypeError):
                buying_power = 0.0
