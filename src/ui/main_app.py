    ...
    def _render_sidebar(self):
        with st.sidebar:
            ...
            if st.session_state.api_key:
                ...
                # Precise numeric entry (supports down to 0.01%)
                threshold_input = st.number_input(
                    "Skip if within % of target",
                    min_value=0.0,
                    max_value=10.0,
                    value=float(st.session_state.rebalance_threshold_pct),
                    step=0.01,
                    format="%.2f",
                    help=threshold_help,
                    key="threshold_number_input",
                )

                # Slider kept in sync for quick/coarse adjustment
                ...
                # Whichever control the user touched last wins; number_input
                # is the source of truth when it differs from the stored value.
                if threshold_input != st.session_state.rebalance_threshold_pct:
